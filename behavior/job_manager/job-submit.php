...
<form action="<?php echo esc_url( $action ); ?>" method="post" id="submit-job-form" class="job-manager-form" enctype="multipart/form-data">
	...
	<?php if ( job_manager_user_can_post_job() || job_manager_user_can_edit_job( $job_id ) ) : ?>
		...
		<?php if ( $company_fields ) : ?>
			<h2><?php esc_html_e( 'פרטי התקשרות', 'wp-job-manager' ); ?></h2>
		...
		<p>
			...
			<!-- <input type="submit" name="submit_job" class="button" value="<?php //echo __('לשליחה', 'wpjm'); ?>" /> -->
			<input type="submit" name="submit_job" class="button" value="<?php echo __('הצג מודעה לפני אישור', 'wpjm'); ?>" />
