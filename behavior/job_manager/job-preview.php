...
<form method="post" id="job_preview" action="<?php echo esc_url( $form->get_action() ); ?>">
	...
	<div class="job_listing_preview_title">
		<input type="submit" name="continue" id="job_preview_submit_button" class="button job-manager-button-submit-listing" value="<?php echo esc_attr( apply_filters( 'submit_job_step_preview_submit_text', __( 'שלח לאישור', 'wp-job-manager' ) ) ); ?>" />
		<input type="submit" name="edit_job" class="button job-manager-button-edit-listing" value="<?php esc_attr_e( 'עריכה', 'wp-job-manager' ); ?>" />
		<h2><?php esc_html_e( 'הצג מודעה לפני אישור', 'wp-job-manager' ); ?></h2>
	</div>
